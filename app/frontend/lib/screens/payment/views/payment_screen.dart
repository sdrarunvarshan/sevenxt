...
import 'package:sevenext/screens/order/views/orders_repository.dart';
import 'package:sevenext/screens/helpers/user_helper.dart';
import 'package:hive_flutter/hive_flutter.dart';
import 'package:razorpay_flutter/razorpay_flutter.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';
import 'package:sevenext/models/coupon_model.dart';
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  bool _isCalculatingSummary = false; // New: To manage loading state for summary
  ...
  void initState() {
    ...
    print('PaymentScreen: _resolvedUserType in initState: $_resolvedUserType'); // ADDED PRINT
          ...
          builder: (context) => ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                subtitle: Text('${address.address}, ${address.city}, ${address.country}'),
                trailing: address.isDefault ? const Chip(label: Text('Default')) : null,
  ...
  double _getItemPrice(ProductModel product) {
    return (product.priceAfetDiscount != null && product.priceAfetDiscount! < product.price)
  ...
  Future<Map<String, dynamic>> _createRazorpayOrder(String orderId) async {
    if (_displayCart == null ||
      ...
      throw Exception('Cannot create Razorpay order without cart details or empty cart');
    }
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) async {
    ...
    try {
      ...
      await _executeOrderPlacement(paymentStatus: 'Paid', paymentId: response.paymentId);
    } catch (e) {
  ...
  Future<void> _placeOrder() async {
    if (_isProcessing || _displayCart == null || _displayCart!.items.isEmpty || _isCalculatingSummary) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No active items in cart or summary not ready')));
      return;
    }
    if (_selectedAddress == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a shipping address'), backgroundColor: Colors.red));
      return;
    }
    final phone = Hive.box('auth').get('user_phone');
    if (phone == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('User phone not found. Please login again.')));
      return;
    }
    ...
    try {
      ...
      final razorpayData = await _createRazorpayOrder("temp"); // "temp" will be replaced by actual orderId from backend later
      _razorpayOrderId = razorpayData['razorpay_order_id'];
      _pendingOrderId = _razorpayOrderId; // Use this as the temporary order ID until confirmed
      ...
      builder: (context) => OtpDialog(
        ...
        onVerify: (String otp) async {
          ...
          try {
            ...
            return {"success": response['success'] == true, "message": response['message'] ?? "Invalid OTP"};
          } catch (e) {
  ...
  void _startRazorpay() {
    final subtotal = _calculateSubtotal();
    final double totalGst = _displayTotalGstAmount ?? 0.0;
    final double totalAmount = subtotal + (_displayShippingFee ?? 0.0) + totalGst - _couponDiscount;
        ...
        openRazorpay(
          ...
          successCallback: (dynamic jsResponse) async {
            ...
            try {
              ...
              _verifiedPaymentMethod = verifyResponse['payment_method'];
              await _executeOrderPlacement(paymentStatus: 'Paid', paymentId: paymentId);
            } catch (e) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Payment verification failed')));
              setState(() => _isProcessing = false);
            }
  ...
  Future<void> _executeOrderPlacement({required String paymentStatus, String? paymentId}) async {
    try {
      ...
      final double totalPrice = subtotal + (_displayShippingFee ?? 0.0) + (_displayTotalGstAmount ?? 0.0) - _couponDiscount;
      ...
      final double? orderStateGstPercent = _displayCart!.items.isNotEmpty ? _displayCart!.items.first.product.stateGstPercent : null;
      final double? orderCentralGstPercent = _displayCart!.items.isNotEmpty ? _displayCart!.items.first.product.centralGstPercent : null;
      ...
      final order = Order(
        ...
        customerName: currentUserName.isNotEmpty ? currentUserName : (_selectedAddress?.name ?? ''),
        ...
        paymentMethod: _verifiedPaymentMethod ?? _selectedPaymentMethod,
        paymentStatus: paymentStatus,
      );
      print('PaymentScreen: Order object created with userType: ${order.userType}'); // ADDED PRINT

      await OrdersRepository.addOrder(order, userType: widget.userType);
      _displayCart!.clearCart();
      if (mounted) Navigator.pushReplacementNamed(context, orderConfirmationScreenRoute, arguments: order);
    } catch (e) {
          ...
          children: [
            ...
            if (_appliedCouponCode != null)
              Row(
                ...
                children: [
                  Text("Coupon $_appliedCouponCode applied", style: const TextStyle(color: Colors.green)),
                  TextButton(onPressed: () => setState(() { _appliedCouponCode = null; _couponDiscount = 0.0; }), child: const Text("Remove")),
                ],
              )
            else
              Row(
                children: [Expanded(child: TextField(controller: controller, decoration: const InputDecoration(hintText: "Enter coupon code"))), const SizedBox(width: 8), SizedBox(width: 90, height: 48, child: ElevatedButton(onPressed: _isApplyingCoupon ? null : () => _applyCoupon(controller.text.trim()), child: _isApplyingCoupon ? const SizedBox(height: 16, width: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Text("Apply"),),)],
              ),
            if (_isLoadingCoupons)
              ...
              const SizedBox(height: 8),
              Column(
                children: _availableCoupons.map((coupon) => ListTile(
                  title: Text(coupon.code),
                  subtitle: Text(coupon.discountType == 'fixed' ? 'Flat ₹${coupon.discountValue} off' : '${coupon.discountValue}% off'),
                  trailing: TextButton(child: const Text("Apply"), onPressed: () => _applyCoupon(coupon.code)),
                )).toList(),
              ),
            ],
          ],
  ...
  Widget _buildAddressSection() {
    if (_isLoadingAddress) return const Center(child: CircularProgressIndicator(color: kPrimaryColor));
    if (_selectedAddress == null) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                children: [
                  const Text('No address found', style: TextStyle(color: Colors.grey)),
                  const SizedBox(height: defaultPadding / 2),
                  ElevatedButton(onPressed: _selectAddress, child: const Text('Add Address')),
                ],
              ),
            ),
          ),
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(_selectedAddress!.name, style: Theme.of(context).textTheme.titleMedium),
                    if (_selectedAddress!.isDefault)
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                        decoration: BoxDecoration(color: kPrimaryColor.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                        child: const Text('Default', style: TextStyle(color: kPrimaryColor, fontSize: 12, fontWeight: FontWeight.bold)),
                      ),
                  ],
                ),
                const SizedBox(height: defaultPadding / 2),
                Text(_selectedAddress!.address, style: const TextStyle(fontSize: 16)),
                const SizedBox(height: 4),
                Text('${_selectedAddress!.city}, ${_selectedAddress!.state}, ${_selectedAddress!.country}, ${_selectedAddress!.postalCode}', style: const TextStyle(fontSize: 16)),
              ],
            ),
          ),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    final double totalAmount = subtotal + (_displayShippingFee ?? 0.0) + stateGst + centralGst - _couponDiscount;
    ...
    return Scaffold(
            ...
            if (hasCart) ...[
                    ...
                    children: [
                      ...
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        ...
                        _isCalculatingSummary
                            ...
                            : Text("₹${(_displayShippingFee ?? 0.0).toStringAsFixed(2)}"),
                      ]),
                      const SizedBox(height: 8),
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        ...
                        _isCalculatingSummary
                            ? const SizedBox(height: 16, width: 16, child: CircularProgressIndicator(strokeWidth: 2))
                            : Text("₹${(_displayStateGstAmount ?? 0.0).toStringAsFixed(2)}"),
                      ]),
                      const SizedBox(height: 8),
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        ...
                        _isCalculatingSummary
                            ? const SizedBox(height: 16, width: 16, child: CircularProgressIndicator(strokeWidth: 2))
                            : Text("₹${(_displayCentralGstAmount ?? 0.0).toStringAsFixed(2)}"),
                      ]),
                      if (_couponDiscount > 0) ...[
                        const SizedBox(height: 8),
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("Coupon Discount"), Text("-₹${_couponDiscount.toStringAsFixed(2)}", style: const TextStyle(color: Colors.green))]),
                      ],
                      const Divider(height: 24),
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Total", style: Theme.of(context).textTheme.titleMedium), Text("₹${totalAmount.toStringAsFixed(2)}", style: Theme.of(context).textTheme.titleMedium)]),
                    ],
                  ),
                ),
              ),
              ...
              ..._paymentMethods.map((method) => Card(
                child: RadioListTile<String>(
                  ...
                  title: Row(children: [Icon(method.icon, color: kPrimaryColor), const SizedBox(width: 8), Text(method.name)]),
                  subtitle: Text(method.description),
                ),
              )).toList(),
            ] else Center(child: Text('No active items in cart', style: Theme.of(context).textTheme.titleMedium)),
          ],
        ),
      ),
      bottomNavigationBar: Padding(
        ...
        child: CartButton(
          ...
          title: "Place Order",
          subTitle: "Pay Now",
          press: (hasCart && _selectedAddress != null && !_isCalculatingSummary) ? _placeOrder : null, // Disable if calculating
          isLoading: _isProcessing || _isCalculatingSummary, // Show loading if processing or calculating summary
        ),
      ),
    );
  }
...
class PaymentMethod {
  ...
  PaymentMethod({required this.id, required this .name, required this.icon, required this.description});
}