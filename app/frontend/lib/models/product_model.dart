...
class ProductModel {
  ...
  final double? priceAfetDiscount; // Actual price to pay (offer price or base price)
  ...
  // ========================= FROM JSON =========================
  factory ProductModel.fromJson(Map<String, dynamic> json, {String userType = 'b2c'}) {
    // ---------- Parse images safely ----------
    ...
    final double compareAtPrice = (json['compare_at_price'] as num?)?.toDouble() ?? 0.0;
    final double basePrice = (isB2B
        ? (json['b2b_price'] as num?)?.toDouble() ?? 0.0
        : (json['b2c_price'] as num?)?.toDouble() ?? 0.0);

    // Parse Dates
    final String? startDateStr = isB2B ? json['b2b_offer_start_date'] : json['b2c_offer_start_date'];
    final String? endDateStr = isB2B ? json['b2b_offer_end_date'] : json['b2c_offer_end_date'];
    
    final DateTime? offerStartDate = startDateStr != null ? DateTime.tryParse(startDateStr) : null;
    final DateTime? offerEndDate = endDateStr != null ? DateTime.tryParse(endDateStr) : null;

    // Check if offer is active based on dates
    final DateTime now = DateTime.now();
    bool isOfferActiveByDate = true;
    if (offerStartDate != null && now.isBefore(offerStartDate)) isOfferActiveByDate = false;
    if (offerEndDate != null && now.isAfter(offerEndDate)) isOfferActiveByDate = false;
    ...
    double strikePrice = (compareAtPrice > 0) ? compareAtPrice : (offerPrice != null ? basePrice : basePrice);
    ...
    return ProductModel(
      ...
      isAvailable: (json['status'] == 'Published' || json['status'] == 'Active') && ((json['stock'] as num?)?.toInt() ?? 0) > 0,
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'stock': stock,
      'status': isAvailable ? 'Published' : 'Archived',
      'rating': rating,
      'reviews': reviews,
      'info': info,
  ...
  // ========================= UI HELPERS =========================
  bool get hasDiscount => priceAfetDiscount != null && priceAfetDiscount! < price;
  ...
  // ========================= OVERRIDES =========================
  @override
  String toString() => 'ProductModel(id: $id, title: $title, price: $finalPrice)';

  @override
  bool operator ==(Object other) => identical(this, other) || other is ProductModel && other.id == id;
